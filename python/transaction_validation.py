...
class ValidationError:
    ...
    code: int  # numeric reject code (batch) or 0 for online
    message: str  # error message text (matches COBOL WS-MESSAGE / FAIL-REASON-DESC)
    field: str = ""  # field that caused the error (online only)
...
class ValidationResult:
    ...
    # Populated by key-field validation when a cross-reference lookup succeeds.
...
def _validate_date_value(date_str: str) -> bool:
    """Replaces the CSUTLDTC call -- checks that the date is a real calendar date.
...
def _validate_amount_format(amt_str: str) -> bool:
...
def validate_online_transaction(
    ...
    # are blanked and will therefore all fail the emptiness checks below.  In
    # our Python equivalent we simply return the key-field error.
    if result.errors:
        return result
...
def _validate_key_fields(
    ...
    if not _is_blank(actid):
        # Account ID path
        if not _is_numeric(actid):
    ...
    elif not _is_blank(cardnum):
        # Card Number path
        if not _is_numeric(cardnum):
            result.is_valid = False
            result.errors.append(
...
def _validate_data_fields(
    ...
    if not _is_numeric(txn.ttype_cd):
        result.is_valid = False
        result.errors.append(
    ...
    if not _is_numeric(txn.tcat_cd):
        result.is_valid = False
        result.errors.append(
    ...
    # --- Amount format ---
    if not _validate_amount_format(txn.tran_amt):
        result.is_valid = False
        result.errors.append(
            ValidationError(
                0,
    ...
    # --- Orig date format ---
    if not _validate_date_format(txn.orig_date):
        result.is_valid = False
        result.errors.append(
            ValidationError(
                0,
    ...
    # --- Proc date format ---
    if not _validate_date_format(txn.proc_date):
        result.is_valid = False
        result.errors.append(
            ValidationError(
                0,
    ...
    if not _validate_date_value(txn.orig_date):
        result.is_valid = False
        result.errors.append(
            ValidationError(
                0,
    ...
    if not _validate_date_value(txn.proc_date):
        result.is_valid = False
        result.errors.append(
            ValidationError(
                0,
    ...
    # --- Merchant ID numeric ---
    if not _is_numeric(txn.merchant_id):
        result.is_valid = False
        result.errors.append(
...
def validate_batch_transaction(
    ...
    if credit_limit < temp_bal:
        ...
        # NOTE: COBOL does NOT short-circuit here; it continues to the
        # expiration check and the last failure wins.  We replicate that.
    ...
    # If overlimit was set but expiration was NOT triggered, keep the overlimit
    # error.  The code above handles both cases because we either:
    # - replaced the list (expiration triggered), or
    # - left the overlimit error in place (expiration not triggered).
